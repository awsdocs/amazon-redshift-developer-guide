...
When you run a COPY command against a table with the EXPLICIT\_IDS option, Amazon Redshift does not check the uniqueness of IDENTITY columns in the table\.  
...
For more information about IDENTITY columns, see [](r_CREATE_TABLE_NEW.md#identity-clause)\. For an example of a COPY command using EXPLICIT\_IDS, see [Load VENUE with explicit values for an IDENTITY column](r_COPY_command_examples.md#r_COPY_command_examples-load-venue-with-explicit-values-for-an-identity-column)\.
